...
<div class="page-content">
  ...
  <el-button type="primary" @click="editCombo_plan">创建</el-button>
</div>
</template>
<script>
...
export default {
  ...
  created () {
    let id = this.combo_planId = this.$route.params.id
    comboAxios.all().then(res => {
      this.comboArr = res
    })
    comboPlanAxios.show(id).then(res => {
      this.combo_plan = res
    })
  },
  methods: {
    editCombo_plan () {
      ...
      let id = this.combo_planId
      this.$refs['ruleForm'].validate(vaild => {
        if (vaild) {
          comboPlanAxios.edit(id, data).then(() => {
